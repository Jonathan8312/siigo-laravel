...
/**
 * A line item as returned on a credit note response.
 */
final class CreditNoteItemDetails
{
    /**
     * @param  list<CreditNoteItemTax>  $taxes
     */
    ...
    public static function fromArray(array $data): self
    {
        ...
        if (is_array($taxes)) {
            foreach ($taxes as $entry) {
                if (is_array($entry)) {
                    $items[] = CreditNoteItemTax::fromArray($entry);
                }
            }
        }
